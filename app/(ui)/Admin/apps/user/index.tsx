...
// ** Types Imports
import { UserData } from 'src/types/apps/userTypes'
...
const columns: GridColDef[] = [
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => {
      return (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <LinkStyled href={{ pathname: `/apps/user/account/${row.id}`, query: { userInfo: JSON.stringify(row) } }}>
            {row.login_id}
          </LinkStyled>
        </Box>
      )
...
const UserList = () => {
  ...
  async function fetchData() {
    try {
      ...
      const rowsWithId = response?.data.data.map((row: UserData, index: number) => ({ ...row, id: `${index}` }))
      ...
      <Grid item xs={12}>
        <Card>
          <CardContent>
            <Grid container spacing={6}>
              <Grid item sm={1} xs={6}>
                <p style={{ fontSize: 'large', fontWeight: 'bold' }}>검색옵션</p>
              </Grid>
              <Grid item sm={4} xs={12}>
                <FormControl fullWidth>
                  <InputLabel id='marketing-select'>마케팅 동의 옵션</InputLabel>
                  <Select
                    fullWidth
                    value={role}
                    id='select-marketing'
                  ...
                  >
                    ...
                    <MenuItem value='agree_marketing'>마케팅 수신 동의</MenuItem>
                    <MenuItem value='disagree-marketing'>마케팅 수신 미동의</MenuItem>
                  </Select>
                </FormControl>
              </Grid>
            </Grid>
