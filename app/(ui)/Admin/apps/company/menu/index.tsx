      ...
      return (
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          >
            <LinkStyled
              href={{
                pathname: `/apps/company/menu/view/${row.idx}`,
                query: { menu: JSON.stringify(row) },
              }}
...
const MenuList = () => {
  ...
  const [selectedOptionFromChild, setSelectedOptionFromChild] =
    useState<number>();
  ...
  async function fetchDataDB() {
    try {
      ...
      const responseBranch = await makeApiCall(
        urlBranch,
        "post",
        1,
        dataBranch
      ...
      const rowsWithId = response?.data.data.map(
        (row: MenuTypes, index: number) => ({ ...row, id: `${index}` })
      );
